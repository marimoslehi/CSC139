...
struct Task {
    ...
    int cpu_burst;
    int priority;
    int waiting_time; // Added waiting time for consistency
};
...
void RoundRobin(struct Task tasks[], int num_tasks, int quantum) {
    int time = 0, taskIndex = 0, tasksCompleted = 0;
    int remainingBurstTime[MAX_TASKS];
    for (int i = 0; i < num_tasks; i++) {
        remainingBurstTime[i] = tasks[i].cpu_burst;
        tasks[i].waiting_time = 0; // Initialize waiting time
    }

    while (tasksCompleted < num_tasks) {
        if (remainingBurstTime[taskIndex] > 0) {
            if (remainingBurstTime[taskIndex] > quantum) {
                time += quantum;
                remainingBurstTime[taskIndex] -= quantum;
            } else {
                time += remainingBurstTime[taskIndex];
                tasks[taskIndex].waiting_time = time - tasks[taskIndex].cpu_burst;
                remainingBurstTime[taskIndex] = 0;
                tasksCompleted++;
            }
        }
        taskIndex = (taskIndex + 1) % num_tasks;
    }
...
void SJF(struct Task tasks[], int num_tasks) {
    sort_by_cpu_burst(tasks, num_tasks); // Assuming this sorts tasks by CPU burst time AND arrival time for tie-breaking
    int currentTime = 0;
    for (int i = 0; i < num_tasks; i++) {
        if (currentTime < tasks[i].arrival_time) currentTime = tasks[i].arrival_time;
...
void Priority_No_Preemption(struct Task tasks[], int num_tasks) {
    ...
    calculate_average_waiting_time(tasks, num_tasks);
}
