...
int main() {
    ...
    int num_tasks, task_id, arrival_time, cpu_burst, priority,time_quantom=0;
    ...
    //we check if the algo is Round Robin if so there will be a time quantom
    if (strcmp(algoName, "RR") == 0) {
        // So we can move the pointer of where the currecnt file is at to the next location by 1 to read the number that will be time quantom
        // its Round Robin
        fseek(file, position + 1, SEEK_SET); 
            //pointer is in the right spot so it reads the number and stores it in the memory location that belongs to time quantom.
            fscanf(file, "%d", &time_quantom);
    }
